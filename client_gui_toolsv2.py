...
# In PyInstaller --onefile mode, __file__ points to temp extract dir
# Use sys._MEIPASS for bundled resources, sys.executable for the exe path
...
def send_message(api_key, model, user_input, session, callback=None):
    ...
    system_prompt = (
        ...
        "You have tool capabilities (terminal, read_file, write_file) configured by the API. Use them when needed."
    )
    
    # Define available tools - must match what the API expects
    tools = [
        {
            "type": "function",
            "function": {
    ...
    try:
        # Explicitly disable proxy to avoid Clash/system proxy issues
        resp = requests.post(
        ...
        if resp.status_code == 200:
            ...
            # Handle tool calls if present (even without tools in request, model may return them)
            ...
            if tool_calls:
                for tc in tool_calls:
                    ...
                    try:
                        import ast
                        args_parsed = ast.literal_eval(args_str) if isinstance(args_str, str) else args_str
                        if isinstance(args_parsed, dict):
                            # Extract the right parameter based on tool name
                            if tool_name == "terminal":
                                args_val = args_parsed.get("command", args_str)
                            elif tool_name == "read_file":
                                args_val = args_parsed.get("path", args_str)
                                # Also get content if present
                                file_content = args_parsed.get("content", "")
                    ...
                    if tool_name == "terminal":
                        try:
                            import subprocess
                            cmd = ["cmd", "/C", args_val] if sys.platform == "win32" else ["bash", "-c", args_val]
                            ...
                            tool_result = "[Terminal error: " + str(e) + "]"
                    elif tool_name == "read_file":
                        try:
                            fp = args_val if os.path.isabs(args_val) else os.path.join(os.getcwd(), args_val)
                            with open(fp, 'r', encoding='utf-8', errors='replace') as fh:
                                tool_result = fh.read()
                            if len(tool_result) > 50000:
                                tool_result = tool_result[:50000] + "\n...(truncated)"
                        except Exception as e:
                            tool_result = "[Read error: " + str(e) + "]"
                    elif tool_name == "write_file":
                        try:
                            fp = args_val if os.path.isabs(args_val) else os.path.join(os.getcwd(), args_val)
                            ...
                            tool_result = "[OK] File written: " + fp
                        except Exception as e:
                            tool_result = "[Write error: " + str(e) + "]"
                    else:
                    ...
                    resp2 = requests.post(API_BASE + "/chat/completions", json=payload2, headers=headers, timeout=60, proxies={'http': None, 'https': None})
...
def main():
    ...
    try:
        ...
        print(f"GUI启动失败: {e}")
        print("切换到命令行模式...")
        # CLI mode not available in GUI build (no sys.stdin)
        print("请在命令行环境下运行此程序。")
        sys.exit(1)
