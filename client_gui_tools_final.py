...
# ─── Configuration ───────────────────────────────────────────────
API_BASE = "https://mimo-v2.5-pro.tp-cook-9d1f9e3a.kb.openai-proxy.com/v1"
...
def send_message(api_key, model, user_input, session, callback=None):
    ...
    system_prompt = (
        "You are Hermes Agent, a diagnostic AI assistant. You have access to tools: terminal (execute shell commands), read_file (read file contents), write_file (write file contents). When the user asks you to run commands, check files, or modify files, use the tool interface. Reply in Chinese. (terminal, read_file, write_file). When asked to execute commands or read/write files, use the tool interface. running from a portable USB drive. "
    ...
    try:
        ...
        if resp.status_code == 200:
            ...
            if "choices" not in data or not data["choices"]:
                reply = "[Error: No choices in response]"
            ...
            # Handle tool calls - execute tools locally
            ...
            if tool_calls:
                for tc in tool_calls:
                    ...
                    try:
                        ...
                        args_val = args_parsed.get("arguments", args_str) if isinstance(args_parsed, dict) else args_str
                    except Exception:
                        args_val = args_str
                    ...
                    resp2 = requests.post(API_BASE + "/chat/completions", json=payload2, headers=headers, timeout=60)
